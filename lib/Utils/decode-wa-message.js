...
export function decodeMessageNode(stanza, meId, meLid) {
    ...
    let remoteJid = jidNormalizedUser(chatId);
    if (remoteJid.endsWith('@lid')) {
        const altJid = fromMe ? addressingContext.recipientAlt : addressingContext.senderAlt;
        if (altJid && jidNormalizedUser(altJid).endsWith('@s.whatsapp.net')) {
            remoteJid = jidNormalizedUser(altJid);
        } else if (isMeLid(remoteJid)) {
            remoteJid = jidNormalizedUser(meId);
        }
    }

    let isSender = fromMe ? meId : (participant || chatId);
    if (isSender) {
        isSender = jidNormalizedUser(isSender);
        if (isSender.endsWith('@lid')) {
            if (addressingContext.senderAlt && jidNormalizedUser(addressingContext.senderAlt).endsWith('@s.whatsapp.net')) {
                isSender = jidNormalizedUser(addressingContext.senderAlt);
            } else if (isMeLid(isSender)) {
                isSender = jidNormalizedUser(meId);
            }
        }
    }

    let isLid = undefined;
    const rawSender = participant || chatId;
    if (rawSender) {
        const normalizedSender = jidNormalizedUser(rawSender);
        if (normalizedSender.endsWith('@lid')) {
            isLid = normalizedSender;
        } else if (addressingContext.senderAlt && jidNormalizedUser(addressingContext.senderAlt).endsWith('@lid')) {
            isLid = jidNormalizedUser(addressingContext.senderAlt);
        } else if (isMe(normalizedSender)) {
            isLid = jidNormalizedUser(meLid);
        }
    }

    let participantPn = participant;
    if (participantPn) {
        participantPn = jidNormalizedUser(participantPn);
        if (participantPn.endsWith('@lid')) {
            if (addressingContext.senderAlt && jidNormalizedUser(addressingContext.senderAlt).endsWith('@s.whatsapp.net')) {
                participantPn = jidNormalizedUser(addressingContext.senderAlt);
            } else if (isMeLid(participantPn)) {
                participantPn = jidNormalizedUser(meId);
            }
        }
    }

    const username = stanza.attrs.notify || 
                     stanza.attrs.peer_recipient_username || 
                     stanza.attrs.recipient_username || 
                     stanza.attrs.participant_username || 
                     undefined;
    ...
    const key = {
        ...
        adressingMode: addressingContext.addressingMode,
