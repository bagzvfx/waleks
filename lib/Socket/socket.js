...
export const makeSocket = (config) => {
    ...
    const onUnexpectedError = (err, msg) => {
        ...
        if (err && err.message && err.message.includes('Bad MAC')) {
            logger.warn('Bad MAC session detected. Forcing pre-keys re-upload to resolve...');
            uploadPreKeysToServerIfRequired(true).catch(e => logger.error({ err: e }, 'failed to force upload pre-keys'));
        }
    };
    ...
    const uploadPreKeysToServerIfRequired = async (force = false) => {
        try {
            ...
            const shouldUpload = lowServerCount || missingCurrentPreKey || force;
            if (shouldUpload) {
                const reasons = [];
                if (lowServerCount)
                ...
                if (force)
                    reasons.push(`forced upload requested`);
